...
@section('main')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="text-white m-0 d-flex align-items-center gap-2">
            ...
            <span class="badge bg-danger bg-opacity-25 text-danger border border-danger border-opacity-25 fs-6 px-2 py-0.5 rounded-pill">
                {{ $logs->total() }}
            </span>
        </h4>
        @if($logs->total() > 0)
    ...
    <div class="table-section">
        ...
        $ip = 'N/A';
                        ...
                        <tr class="transition-row">
                            ...
                            <td>
                                <span class="badge font-monospace text-dark fw-bold {{ $method == 'POST' ? 'bg-warning' : ($method == 'GET' ? 'bg-info' : 'bg-danger text-white') }}">
                                    {{ $method }}
                                </span>
                            </td>
                            ...
                            <td>
                                <span class="badge badge-role {{ $userId == 'Guest' ? 'badge-user opacity-75' : 'badge-admin' }}">
                                    {{ $userId }}
                                </span>
                            </td>
                            <td class="text-center">
                                {{-- الزرار شغال وبيرمي على الترقيم المظبوط --}}
                                <button type="button" class="btn btn-outline-info btn-sm rounded-circle px-2 py-1 shadow-sm"
                                        data-bs-toggle="modal" data-bs-target="#logModal{{ $index }}" title="View Full Details">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </td>
                        </tr>
    ...
    @foreach ($logs as $index => $log)
        ...
        <div class="modal fade" id="logModal{{ $index }}" tabindex="-1" aria-labelledby="logModalLabel{{ $index }}" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content text-white" style="background: rgba(20, 24, 45, 0.98); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1);">
                    <div class="modal-header border-bottom border-opacity-10 border-white">
                        <h5 class="modal-title d-flex align-items-center gap-2 text-info" id="logModalLabel{{ $index }}">
                    ...
                    <div class="modal-body">
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <span class="badge {{ $method == 'POST' ? 'bg-warning text-dark' : 'bg-info text-dark' }} fw-bold">{{ $method }}</span>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-12">
                                ...
                                <div class="p-2 rounded bg-black bg-opacity-40 text-info font-monospace small text-break">{{ $url }}</div>
                            </div>
                            <div class="col-12">
                                <small class="text-muted d-block">User Agent</small>
                                <div class="p-2 rounded bg-secondary bg-opacity-10 text-white-50 small font-monospace">{{ $userAgent }}</div>
                            </div>
                        </div>

                        <div>
                            <h6 class="text-success border-bottom border-success border-opacity-25 pb-2 mb-2 d-flex align-items-center gap-2">
                    ...
                    <div class="modal-footer border-top border-opacity-10 border-white">
                        <button type="button" class="btn btn-secondary btn-sm px-3" data-bs-dismiss="modal">Close View</button>
                    </div>
                </div>
            </div>
        </div>
